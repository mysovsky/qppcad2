...
bool compare_string_decomps(const std::vector<std::string> &s1,
                            const std::vector<std::string> &s2, int n) {
...
plugin_param_t::plugin_param_t(basic_types t, const std::string &n,
                               const std::string &d)
...
void python_plugin_t::load_header() {
  ...
  std::string err = extract_json(data, "menu name", plug_menu_name) +
  ...
  try {
    for (json::iterator it = jparams.begin(); it != jparams.end(); ++it) {
      ...
      auto p = std::make_shared<plugin_param_t>(t, it.key(),
  ...
  std::sort(
      ...
      [](std::shared_ptr<plugin_param_t> p1,
         std::shared_ptr<plugin_param_t> p2) { return p1->pos < p2->pos; });
...
plugin_tree_t::plugin_tree_t(const std::string &_name) { name = _name; }

void plugin_tree_t::build_nested(std::vector<std::vector<std::string>> &list,
                                 int lvl) {
  if (list[0].size() == lvl) {
    module_name = join_strings(list[0], ".");
    return;
  }
  std::vector<std::string> module_name_decomp(list[0].begin(),
                                              list[0].begin() + lvl);
  ...
  while (!list.empty()) {
    ...
    for (int i : idcs)
      sublist.push_back(
          std::vector<std::string>(list[i].begin(), list[i].end()));
...
void plugin_tree_t::bind_plugins(
    const std::vector<std::shared_ptr<python_plugin_t>> &pluglist) {
  if (nested.empty()) {
    for (auto p : pluglist)
...
plugin_manager_t::plugin_manager_t(const std::string &_plugdir)
  ...
  if (!fs::exists(plug_path / "__init__.py") ||
      !fs::exists(plug_path / "plugins.json")) {
    status = plugmgr_plugins_missing;
    error_msg =
        "Plugin folder either empty, corrupted or misplaced:\n{}\nPlugins will "
        "be unavailable.\nCheck Edit->Setting->General.\n";
    error_descr = error_msg;
    return;
  }
...
void plugin_manager_t::locate_plugins() {
  ...
  for (const fs::directory_entry &dir_entry :
       fs::recursive_directory_iterator(plugdir)) {
    std::vector<std::string> entry_path_decomp(dir_entry.path().begin(),
                                               dir_entry.path().end());
    std::vector<std::string> relpath_decomp(entry_path_decomp.begin() +
                                                plug_dir_decomp.size() - 1,
    ...
    if (dir_entry.path().extension() == ".json" &&
      ...
      *(relpath_decomp.end() - 1) = dir_entry.path().stem();
      std::shared_ptr<python_plugin_t> p = std::make_shared<python_plugin_t>(
          dir_entry.path().stem(), join_strings(relpath_decomp, "."),
          dir_entry.path());
...
void plugin_manager_t::load_plugins() {
  ...
  if (status == plugmgr_hdr_error) {
    error_msg =
        error_msg + "There where errors in plugin header files.\nSome plugins "
                    "will be unavailable.\nCheck python console for details\n";
    error_descr = error_descr +
                  "There where errors in plugin header files.\nSome plugins "
                  "will be unavailable.\nCheck python console for details\n";
  }
  py::exec("import sys");
  ...
  if (status == plugmgr_load_error) {
    error_msg =
        error_msg + "There were errors during plugins import.\nSome plugins "
                    "will be unavailable.\nCheck python console for details\n";
    error_descr = error_descr +
                  "There were errors during plugins import.\nSome plugins will "
                  "be unavailable.\nCheck python console for details\n";
  }
}
