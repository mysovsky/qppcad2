...
bool python_manager_t::execute(std::string command) {
  ...
  try {
    ...
    bool comm_is_multiline =
        std::count(command.begin(), command.end(), '\n') > 0;
    bool comm_has_statement = command.find("for") != std::string::npos ||
                              command.find("import") != std::string::npos;
...
void python_manager_t::get_completion_list(QString text, QStringList &sl) {
  ...
  std::string command =
      fmt::format("complete_text(\"{}\")", text.toStdString());
  py::list retlist;
