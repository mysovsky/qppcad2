...
geom_view_t::geom_view_t(): ws_item_t() {
  ...
  m_geom = std::make_shared<xgeometry<float> >(3,"rg1");
...
// asm
geom_view_t::geom_view_t(std::shared_ptr<xgeometry<float> > g,
			 bool hardcoded){
...
bool geom_view_t::mouse_click(ray_t<float> *click_ray, bool pressed) {
  ...
  if (click_ray && m_geom && m_parent_ws) {
      ...
      if (!res.empty()) {
		...
		astate -> tlog("Atom selected {} {}",res[0].m_atm, res[0].m_idx);
...
void geom_view_t::sel_atom(int atom_id, index atom_idx) {
  ...
  app_state_t* astate = app_state_t::get_inst();
  astate->make_viewport_dirty();
  size_t atom_tp = m_geom->typetable()->type(atom_id);
  ...
  if (atom_id >= 0 && atom_id < m_geom->nat()) {
      ...
      if (m_geom->nat() < 1000) {
          astate->tlog(">> SEL: {}{:<8} {:<8}{:>16.5f} {:>16.5f} {:>16.5f} {}",
                       m_geom->typetable()->atom_name(atom_tp),
                       atom_id,atom_tp,
...
void geom_view_t::sel_by_type(const int item_type_to_select) {
  ...
  for (auto i = 0; i < m_geom->nat(); i++)
    if (m_geom->typetable()->type(i) == item_type_to_select) sel_atom(i);
...
void geom_view_t::unsel_by_type(const int item_type_to_unselect) {
  ...
  for (auto i = 0; i < m_geom->nat(); i++)
    ...
    if (m_geom->typetable()->type(i) == item_type_to_unselect) unsel_atom(i);
...
void geom_view_t::ins_atom(const int atom_type, const vector3<float> &pos) {
  ...
  m_geom->add(m_geom->typetable()->atomic_type(atom_type), pos);
...
void geom_view_t::swap_atoms(const size_t at1, const size_t at2, bool swap_names) {
  ...
  std::string atom1_name = swap_names ? m_geom->typetable()->atom_name(at1) : m_geom->typetable()->atom_name(at2);
  std::string atom2_name = swap_names ? m_geom->typetable()->atom_name(at2) : m_geom->typetable()->atom_name(at1);
...
void geom_view_t::copy_from_xgeom(xgeometry<float > &xgeom_inst) {
...
void geom_view_t::copy_to_xgeom(xgeometry<float> &xgeom_inst,
                                bool copy_selected,
                                bool copy_cell) {
  if (!m_geom) return;
  ...
  if (copy_cell) {
    ...
    xgeom_inst.cell->DIM = m_geom->DIM();
...
void geom_view_t::save_to_json(json &data) {
  ...
  for (auto &record: m_tws_tr->m_bonding_table.m_dist) {
      ...
      bt_rec.push_back(m_geom->typetable()->atomic_type(record.first.m_a));
      bt_rec.push_back(m_geom->typetable()->atomic_type(record.first.m_b));
...
void geom_view_t::load_from_json(json &data, repair_connection_info_t &rep_info) {
  ...
  if (auto val_itr = data.find(JSON_GEOM_VIEW_BONDING_TABLE); val_itr != data.end()) {
	...
	int type1 = m_geom->typetable()->type_of_atom(elem[0].get<std::string>());
          int type2 = m_geom->typetable()->type_of_atom(elem[1].get<std::string>());
