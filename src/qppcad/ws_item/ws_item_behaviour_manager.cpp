...
void ws_item_behaviour_manager_t::load_fixtures_from_path(
  ...
  for (const auto &rec : file_paths) {
    ...
    QDirIterator fxts_dir(QString::fromStdString(rec),
    ...
    while (fxts_dir.hasNext()) {
      ...
      if (check_file.exists() && check_file.isFile()) {
        ...
        new_fxt.load_from_file(fxt_manifest_fn.toStdString(),
                               fxt_dir.toStdString());
...
std::shared_ptr<ws_item_t> ws_item_behaviour_manager_t::load_ws_itm_from_file(
    const std::string &file_name, size_t io_bhv_idx, workspace_t *ws) {
  ...
  auto new_ws_item =
      fbr_ws_item_by_type(m_ws_item_io[io_bhv_idx]->m_accepted_type);
...
ws_item_behaviour_manager_t::load_ws_itm_from_file(const std::string &file_name,
                                                   workspace_t *ws) {
...
bool ws_item_behaviour_manager_t::save_ws_itm_to_file(
    std::string &file_name, std::shared_ptr<ws_item_t> ws_item, size_t bhv_id,
    std::string &message) {
  ...
  if (bhv_id < m_ws_item_io.size() && m_ws_item_io[bhv_id]->can_save() &&
    ...
    bool check =
        m_ws_item_io[bhv_id]->check_before_save(ws_item.get(), message);
    if (check) {
...
std::string
ws_item_behaviour_manager_t::get_ff_full_name(size_t _file_format_hash) {
...
std::string
ws_item_behaviour_manager_t::get_ff_short_name(size_t _file_format_hash) {
...
size_t ws_item_behaviour_manager_t::reg_ff(
    std::string _full_name, std::string _short_name,
  ...
  if (it == m_file_formats.end()) {
    ...
    if (it_ffg != m_file_format_groups.end())
      m_file_format_groups[_file_format_group_hash].m_ffs_lookup.insert(
          _ff_hash);
...
size_t ws_item_behaviour_manager_t::reg_ffg(std::string _full_name,
  ...
  if (it == m_file_format_groups.end()) {
    ...
    m_file_format_groups.emplace(_file_format_group_hash,
...
size_t ws_item_behaviour_manager_t::reg_tool(
    std::string _full_name, size_t _g_hash, size_t _t_hash, bool _itm_req,
...
size_t ws_item_behaviour_manager_t::reg_reg_sf_fbr(
    std::string _full_name, size_t _g_hash,
...
void ws_item_behaviour_manager_t::exec_tool_by_name(std::string tool_name,
  ...
  auto it = std::find_if(m_tools_info.begin(), m_tools_info.end(),
                         [&tool_name](const auto &e) {
                           return e.second.m_full_name == tool_name;
                         });

  if (it != m_tools_info.end())
    exec_tool(item, it->first, _error_ctx);
}
...
std::optional<size_t> ws_item_behaviour_manager_t::get_ff_by_short_name(
  ...
  auto find_iter =
      std::find_if(std::begin(m_file_formats), std::end(m_file_formats),
...
std::optional<size_t>
ws_item_behaviour_manager_t::get_io_bhv_by_file_format(size_t file_format) {
...
std::optional<size_t>
ws_item_behaviour_manager_t::get_io_bhv_by_file_format_ex(size_t file_format,
                                                          size_t type_hash) {
  for (size_t i = 0; i < m_ws_item_io.size(); i++)
    if (m_ws_item_io[i]->m_accepted_file_format == file_format &&
...
void ws_item_behaviour_manager_t::reg_io_bhv(
    ...
    size_t accepted_file_format, size_t accepted_type) {
...
void ws_item_behaviour_manager_t::reg_item_fbr(
    size_t hash, std::function<std::shared_ptr<ws_item_t>()> func) {
...
void ws_item_behaviour_manager_t::reg_ext_editor_fbr(
    size_t hash, size_t editor_order, std::string editor_name,
...
void ws_item_behaviour_manager_t::reg_toolbar_elem_fbr(
    size_t hash, std::string editor_name,
...
std::shared_ptr<ws_item_obj_insp_widget_t>
ws_item_behaviour_manager_t::get_obj_insp_widget_sp(size_t hash) {
...
std::shared_ptr<ws_item_extended_editor_t>
ws_item_behaviour_manager_t::get_ext_editor_widget_sp(size_t hash,
                                                      size_t ed_order) {
...
std::shared_ptr<ws_item_t>
ws_item_behaviour_manager_t::fbr_ws_item_by_type(size_t type_id) {
...
std::shared_ptr<ws_item_t>
ws_item_behaviour_manager_t::fbr_ws_item_by_name(const std::string &type_name) {

  app_state_t *astate = app_state_t::get_inst();
  return fbr_ws_item_by_type(
      astate->hash_reg->calc_hash_ub<std::string>(type_name));
}
