...
ws_viewer_widget_t::ws_viewer_widget_t(QWidget *parent)
  ...
  connect(this, &ws_viewer_widget_t::customContextMenuRequested, this,
...
void ws_viewer_widget_t::draw_text_logo(QPainter &painter) {
  ...
  QString logo_string =
      tr("qpp::cad, git rev.:%1")
          .arg(QString::fromStdString(build_info_helper::get_git_version()));

  int c_x = painter.window().width() - fm.horizontalAdvance(logo_string) - 10;
...
void ws_viewer_widget_t::paintGL() {
  ...
  glapi->glViewport(static_cast<int>(0), static_cast<int>(0),
  ...
  if (astate->m_show_debug_frame_stats) {
    ...
    painter.drawText(3, 3, 280, 30, Qt::AlignLeft,
                     QString::fromStdString(fmt::format(
                         "Frame time GPU: {:6.6f} ms.",
                         (astate->m_last_frame_time_gpu) / 1000000.0)));

    painter.drawText(3, 30, 280, 30, Qt::AlignLeft,
                     QString::fromStdString(fmt::format(
                         "Frame time CPU: {:6.6f} ms.",
                         (astate->m_last_frame_time_cpu) / 1000000.0)));
  }
  ...
  if (!astate->m_immersive_mode && !astate->m_ignore_scanline)
...
void ws_viewer_widget_t::mouseMoveEvent(QMouseEvent *event) {
  ...
  if (event) {
    ...
    astate->mouse_y_dc =
        (0.5f - astate->mouse_y / float(this->height())) * 2.0f;
    ...
    if (astate->camera && astate->mouse_lb_pressed &&
        (kb_mod & Qt::ControlModifier)) {
...
void ws_viewer_widget_t::wheelEvent(QWheelEvent *event) {
  ...
  if (astate->camera) {
    astate->camera->update_camera_zoom(event->angleDelta().y() / 180.0f *
                                       astate->m_middle_mb_translate_mode);
    astate->camera->update_camera();
    astate->make_viewport_dirty();
  }
}
