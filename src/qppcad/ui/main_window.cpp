...
main_window_t::main_window_t(QWidget *parent) : QMainWindow(parent) {
  ...
  connect(astate->astate_evd, &app_state_event_disp_t::wss_changed_signal, this,
          &main_window_t::wss_changed_slot);

  connect(astate->astate_evd, &app_state_event_disp_t::cur_ws_changed_signal,
          this, &main_window_t::cur_ws_changed);

  connect(astate->astate_evd,
          &app_state_event_disp_t::cur_ws_selected_item_changed_signal, this,
          &main_window_t::cur_ws_selected_item_changed);

  connect(astate->astate_evd,
          &app_state_event_disp_t::cur_ws_edit_type_changed_signal, this,
          &main_window_t::cur_ws_edit_type_changed);

  connect(astate->astate_evd,
          &app_state_event_disp_t::
              cur_ws_selected_atoms_list_selection_changed_signal,
          this, &main_window_t::cur_ws_selected_atoms_list_selection_changed);

  connect(astate->astate_evd, &app_state_event_disp_t::new_file_loaded_signal,
          this, &main_window_t::rebuild_recent_files_menu);
...
construct_from_geom1(workspace_t &ws, std::shared_ptr<xgeometry<float>> geom,
                     const std::string &name) {

  auto new_item = ws.m_owner->m_bhv_mgr->fbr_ws_item_by_type(
  ...
  for (auto ptr : p->nested) {
    ...
    if (ptr->nested.empty()) {
      ...
      if (ptr->plugin->status != python_plugin_t::plugin_status_ok)
        QObject::connect(plugin_act, &QAction::triggered, [s, ptr]() {
          QMessageBox::warning(nullptr, s.c_str(),
                               ptr->plugin->error_msg.c_str());
        });
      else {
        QObject::connect(plugin_act, &QAction::triggered, [s, ptr, astate]() {
          ...
          if (parm_dialog.result() == QDialog::Accepted)
            try {
              ...
              if (res != "")
                QMessageBox::information(nullptr, s.c_str(), res.c_str());
            } catch (py::error_already_set &err) {
              astate->make_viewport_dirty();
              QMessageBox::warning(nullptr, s.c_str(), err.what());
            }
        });
      }
...
void main_window_t::init_menus() {
  ...
  connect(file_menu_new_ws, &QAction::triggered, this,
  ...
  connect(file_menu_close_ws, &QAction::triggered, this,
  ...
  connect(file_menu_close_all_ws, &QAction::triggered, this,
  ...
  connect(file_menu_open_ws, &QAction::triggered, this,
  ...
  connect(file_menu_save_ws, &QAction::triggered, this,
  ...
  connect(file_menu_save_ws_as, &QAction::triggered, this,
          &main_window_t::save_ws_as);

  file_menu->addSeparator();
  file_menu_import_as_new_ws =
      file_menu->addMenu(tr("Import as new workspace"));
  file_menu_import_to_cur_ws =
      file_menu->addMenu(tr("Import to current workspace"));
  ...
  connect(edit_menu_ptable_widget, &QAction::triggered, []() {
    ptable_rich_widget_t ptable_rich_wdgt;
    ptable_rich_wdgt.exec();
  ...
  connect(edit_menu_selection_select_all, &QAction::triggered, this,
  ...
  connect(edit_menu_selection_unselect_all, &QAction::triggered, this,
  ...
  connect(edit_menu_selection_invert, &QAction::triggered, this,
  ...
  connect(edit_menu_switch_ws_edit_mode, &QAction::triggered, this,
  ...
  connect(ws_menu_rename_ws, &QAction::triggered, this,
  ...
  connect(ws_create_geometry, &QAction::triggered, this,
          &main_window_t::action_select_all_content);
  ...
  connect(ws_create_symmetry, &QAction::triggered, this,
          &main_window_t::action_select_all_content);
  ...
  connect(ws_create_gprimitive, &QAction::triggered, this,
          &main_window_t::action_select_all_content);
  ...
  connect(ws_create_arrow_array, &QAction::triggered, this,
  ...
  connect(ws_menu_bg_color, &QAction::triggered, this,
  ...
  connect(ws_copy_cam, &QAction::triggered, this,
  ...
  connect(view_menu_show_oi, &QAction::toggled, this,
  ...
  connect(view_menu_show_gizmo, &QAction::toggled, this,
  ...
  connect(view_menu_toggle_fullscreen, &QAction::toggled, this,
  ...
  connect(view_menu_settings, &QAction::triggered, []() {
  ...
  connect(view_menu_log_wdgt, &QAction::toggled, this,
  ...
  view_menu_console->setShortcutContext(
      Qt::ShortcutContext::ApplicationShortcut);
  connect(view_menu_console, &QAction::triggered, this,
          &main_window_t::action_toggle_console);
  ...
  connect(view_menu_toggle_debug_info, &QAction::toggled, [](bool checked) {
    app_state_t *astate = app_state_t::get_inst();
  ...
  connect(view_menu_toggle_debug_tws_tree, &QAction::toggled, [](bool checked) {
    app_state_t *astate = app_state_t::get_inst();
  ...
  if (astate->plug_mgr->status ==
          plugin_manager_t::plugmgr_plugin_folder_not_found ||
      astate->plug_mgr->status == plugin_manager_t::plugmgr_plugins_missing) {
    QMessageBox::warning(nullptr, QString("Plugin error!"),
    ...
    connect(plugins_manage, &QAction::triggered, this,
  ...
  connect(help_menu_about, &QAction::triggered, []() {
    QMessageBox::about(
        nullptr, "Fenzihua",
        QObject::tr("Site : <a href='https://github.com/mysovsky/fenzihua'>"
            ...
            .arg(QString::fromStdString(
                build_info_helper::get_python_version())));
  });
...
void main_window_t::init_widgets() {
  ...
  tp_print_screen->setIconSize(
      QSize(astate->size_guide.tool_panel_icon_size(),
  ...
  connect(tp_print_screen, &QPushButton::pressed, this,
  ...
  connect(tp_edit_mode,
          ...
          this, &main_window_t::ws_edit_mode_selector_button_clicked);
  ...
  tp_edit_mode_item->setIconSize(
      QSize(astate->size_guide.tool_panel_icon_size(),
            astate->size_guide.tool_panel_icon_size()));
  tp_edit_mode_item->setToolTip(
      tr("Set current workspace's edit mode to <edit item>"));
  ...
  tp_edit_mode_content->setIconSize(
      QSize(astate->size_guide.tool_panel_icon_size(),
            astate->size_guide.tool_panel_icon_size()));
  tp_edit_mode_content->setToolTip(
      tr("Set current workspace's edit mode to <edit content>"));
  ...
  connect(tp_camera_tool, &QToolButton::triggered, this,
  ...
  tp_scenic_rot_cam->setIcon(
      QIcon("://images/baseline-3d_rotation_x-24px.svg"));
  ...
  tp_scenic_rot_cam->setIconSize(
      QSize(astate->size_guide.tool_panel_icon_size(),
            astate->size_guide.tool_panel_icon_size()));
  tp_scenic_rot_cam->setCheckable(true);
  connect(tp_scenic_rot_cam, &QPushButton::toggled, this,
  ...
  tp_anim_fast_forward->setIconSize(
      QSize(astate->size_guide.tool_panel_icon_size(),
            astate->size_guide.tool_panel_icon_size()));
  tp_anim_fast_forward->setToolTip(
      tr("Fast-forward to last frame of animated geometry"));
  tp_anim_fast_forward->setIcon(
      QIcon("://images/outline-fast_forward-24px.svg"));
  connect(tp_anim_fast_forward, &QPushButton::clicked, this,
  ...
  tp_force_sel_lbl_vis->setIconSize(
      QSize(astate->size_guide.tool_panel_icon_size(),
            astate->size_guide.tool_panel_icon_size()));
  tp_force_sel_lbl_vis->setToolTip(tr("Force labels selective visibility"));
  tp_force_sel_lbl_vis->setIcon(
      QIcon("://images/outline-font_download-24px.svg"));
  connect(tp_force_sel_lbl_vis, &QPushButton::toggled, this,
  ...
  tp_add_cube->setIconSize(QSize(astate->size_guide.tool_panel_icon_size(),
                                 astate->size_guide.tool_panel_icon_size()));
  connect(tp_add_cube, &QPushButton::clicked, this,
  ...
  tp_add_arrow->setIconSize(QSize(astate->size_guide.tool_panel_icon_size(),
                                  astate->size_guide.tool_panel_icon_size()));
  connect(tp_add_arrow, &QPushButton::clicked, this,
  ...
  tp_add_point_sym_group->setFixedHeight(
      astate->size_guide.tool_panel_ctrl_h());
  ...
  tp_add_point_sym_group->setIconSize(
      QSize(astate->size_guide.tool_panel_icon_size(),
            astate->size_guide.tool_panel_icon_size()));
  connect(tp_add_point_sym_group, &QPushButton::clicked, this,
  ...
  extended_editor_compositor =
      new ws_item_extended_editor_compositor_t(nullptr);
}
...
void main_window_t::init_layouts() {
  ...
  splitter_ws_viewer_py_console_log->addWidget(
      splitter_ws_viewer_extended_editor);
...
void main_window_t::dragLeaveEvent(QDragLeaveEvent *event) { event->accept(); }
...
void main_window_t::open_ws() {
  ...
  QString file_name = QFileDialog::getOpenFileName(
      this, "Open qpp::cad workspace", astate->m_last_dir, "*.json");
  ...
  if (ok) {
    ...
    if (check_file.exists() && check_file.isFile() && cur_ws->m_fs_path != "" &&
      ...
      QString file_name = QFileDialog::getSaveFileName(
          this, "Save qpp::cad workspace", astate->m_last_dir, "*.json");
...
void main_window_t::close_cur_ws() {
  ...
  if (ok) {
    ...
    reply =
        QMessageBox::question(this, tr("Workspace -> Close"),
...
void main_window_t::rename_cur_ws() {
  ...
  if (ok) {
    ...
    QString text = QInputDialog::getText(
        this, tr("Workspace -> Rename"), tr("User name:"), QLineEdit::Normal,
...
void main_window_t::change_cur_ws_bg() {
  ...
  if (ok) {
    ...
    const QColor clr = QColorDialog::getColor(
        _stored_color, this, "Select workspace`s background color");
    if (clr.isValid()) {

      cur_ws->m_background_color = vector3<float>{
          static_cast<float>(clr.redF()), static_cast<float>(clr.greenF()),
...
void main_window_t::cur_ws_selected_item_changed() {
  ...
  auto [cur_ws, cur_itm, as_al] =
      astate->ws_mgr->get_sel_tpl_itm<geom_view_t>();
...
void main_window_t::cur_ws_selected_atoms_list_selection_changed() {
  ...
  if (ok) {
    ...
    if (as_al->m_atom_idx_sel.size() == 2 &&
    ...
    if (as_al->m_atom_idx_sel.size() == 3 &&
      ...
      auto cur_sel = as_al->m_measure->is_angle_msr_exists(
          as_al->m_atom_ord_sel[0].m_atm, as_al->m_atom_ord_sel[1].m_atm,
          as_al->m_atom_ord_sel[2].m_atm, as_al->m_atom_ord_sel[0].m_idx,
          as_al->m_atom_ord_sel[1].m_idx, as_al->m_atom_ord_sel[2].m_idx);
...
void main_window_t::tp_dist_button_clicked(bool checked) {
  ...
  if (as_al->m_atom_idx_sel.size() == 2 &&
    ...
    if (checked)
      as_al->m_measure->add_bond_msr(it1->m_atm, it2->m_atm, it1->m_idx,
                                     it2->m_idx);

    else
      as_al->m_measure->rm_bond_msr(*cur_sel);
  }
...
void main_window_t::tp_force_sel_lbl_vis_button_clicked(bool checked) {
  ...
  // if selective labels rendering unchecked - force it and select some random
  // style
...
void main_window_t::tp_toggle_atom_override_button_clicked(bool checked) {
  ...
  auto [cur_ws, cur_item, as_al, ok] =
      astate->ws_mgr->get_sel_tpl_itmc<geom_view_t>();
  if (!ok || cur_ws->m_edit_type == ws_edit_e::edit_item)
    return;
  ...
  for (auto &rec : as_al->m_atom_idx_sel)
    if (rec.m_atm < as_al->m_geom->nat()) {
      ...
      if (as_al->m_geom->xfield<float>(xgeom_atom_r, rec.m_atm) < 0.01f) {
        ...
        float _rad = 1.0f;
        if (ap_idx)
          _rad = ptable::get_inst()->arecs[*ap_idx - 1].m_radius;
...
void main_window_t::tp_add_arrow_clicked() { simple_query::embed_arrow(); }

void main_window_t::tp_add_cube_clicked() { simple_query::embed_cube(); }
...
void main_window_t::stop_update_cycle() {
  ...
  if (ws_viewer_widget && ws_viewer_widget->m_update_timer) {
    ...
    p_elapsed_time_in_event_loop =
        ws_viewer_widget->m_update_timer->remainingTime();
  ...
  for (int i = 0; i < astate->m_recent_files.size(); i++) {
    ...
    if (!astate->m_recent_files[i].m_native)
      rec_menu_entry = fmt::format(
          "{}) {} ({})", i, astate->m_recent_files[i].m_file_name, ff_name);
    else
      rec_menu_entry = fmt::format("{}) {} (qpp::cad workspace)", i,
                                   astate->m_recent_files[i].m_file_name);

    file_menu_recent_entries[i]->setText(
        QString::fromStdString(rec_menu_entry));
  }
}
...
void main_window_t::recent_files_clicked() {
  ...
  for (int i = 0; i < file_menu_recent_entries.size(); i++)
    if (file_menu_recent_entries[i]->isVisible() &&
        file_menu_recent_entries[i] == obj) {
      idx = i;
    }
  ...
  if (idx != -1 && idx < astate->m_recent_files.size()) {
    ...
    if (rec_idx.m_native)
      ...
      auto bhv_id =
          astate->ws_mgr->m_bhv_mgr->get_io_bhv_by_file_format(rec_idx.m_ff_id);
      if (bhv_id)
        astate->ws_mgr->import_from_file(rec_idx.m_file_name, *bhv_id, true);
      else
        astate->ws_mgr->load_from_file_autodeduce(rec_idx.m_file_name);
    }
  }
}
...
void main_window_t::build_bhv_menus_and_actions() {
  ...
  for (auto &ff_grp : bhv_mgr->m_file_format_groups) {
    ...
    QMenu *new_menu_as_new_ws = file_menu_import_as_new_ws->addMenu(
        QString::fromStdString(ff_grp.second.m_full_name));

    QMenu *new_menu_to_cur_ws = file_menu_import_to_cur_ws->addMenu(
        QString::fromStdString(ff_grp.second.m_full_name));

    QMenu *new_menu_export_selected = file_menu_export_sel_as->addMenu(
        QString::fromStdString(ff_grp.second.m_full_name));
    ...
    file_menu_export_sel_as_menus.emplace(ff_grp.first,
                                          new_menu_export_selected);
      ...
      for (size_t i = 0; i < bhv_mgr->m_ws_item_io.size(); i++) {
        ...
        if (bhv_mgr->m_ws_item_io[i]->m_accepted_file_format == ff &&
          ...
          connect(new_act, &QAction::triggered, this,
                  &main_window_t::action_bhv_import_to_cur_workspace);
          new_act->setText(
        ...
        if (bhv_mgr->m_ws_item_io[i]->m_accepted_file_format == ff &&
          ...
          new_act->m_joined_data[0] = i;
          connect(new_act, &QAction::triggered, this,
                  &main_window_t::action_bhv_import_as_new_workspace);
          new_act->setText(
              QString::fromStdString(bhv_mgr->m_file_formats[ff].m_full_name));
        ...
        if (bhv_mgr->m_ws_item_io[i]->m_accepted_file_format == ff &&
          ...
          new_act->m_joined_data[0] = i;
          connect(new_act, &QAction::triggered, this,
                  &main_window_t::action_bhv_export_selected);
          new_act->setText(
              QString::fromStdString(bhv_mgr->m_file_formats[ff].m_full_name));
...
void main_window_t::build_bhv_tools_menus() {
  ...
  for (auto &ffg : astate->ws_mgr->m_bhv_mgr->m_tools_groups) {
    QMenu *new_menu =
        tools_menu->addMenu(QString::fromStdString(ffg.second.m_full_name));
  ...
  for (auto &ff : astate->ws_mgr->m_bhv_mgr->m_tools_info) {
    ...
    connect(new_act, &QAction::triggered, this,
            &main_window_t::action_bhv_tools_menus_clicked);
    ...
    if (it_g != tools_menu_groups.end())
      it_g->second->addAction(new_act);
    else
      tools_menu->addAction(new_act);
  ...
  for (auto elem : tools_menu_actions) {
    ...
    if (cur_ws) {
      ...
      if (it_t != bhv_mgr->m_tools_info.end()) {
        ...
        if (!it_t->second.m_item_required)
...
void main_window_t::action_bhv_import_to_cur_workspace() {
  ...
  if (b_id < bhv_mgr->m_ws_item_io.size() &&
    ...
    std::string file_name =
        QFileDialog::getOpenFileName(this, "Import to current workspace",
                                     astate->m_last_dir, "*")
            .toStdString();
    if (!file_name.empty())
...
void main_window_t::action_bhv_import_as_new_workspace() {
  ...
  if (b_id < bhv_mgr->m_ws_item_io.size() &&
    ...
    std::string file_name =
        QFileDialog::getOpenFileName(this, "Import as new workspace",
                                     astate->m_last_dir, "*")
            .toStdString();
    if (!file_name.empty())
...
void main_window_t::action_bhv_export_selected() {
  ...
  if (b_id < bhv_mgr->m_ws_item_io.size() &&
    ...
    QString qfile_name = QFileDialog::getSaveFileName(this, "Export selected",
                                                      astate->m_last_dir);
    ...
    if (cur_it) {
      ...
      for (auto &exp_act : file_menu_export_sel_as_acts) {
        ...
        if (bhv_mgr->m_ws_item_io[bhv_id]->can_save() &&
            bhv_mgr->m_ws_item_io[bhv_id]->m_accepted_type ==
                cur_it->get_type()) {
...
void main_window_t::slot_shortcut_terminate_app() { QApplication::quit(); }
