...
create_arrow_array_widget_t::create_arrow_array_widget_t() {
    ...
    if (cur_item) {
      ...
      if (cur_gv) {
        ...
        if (anim && anim->get_current_anim()) {
          ...
          astate->log(
              fmt::format("anim= {} total = {} fr tot= {} fr cur = {}\n", anim,
                          anim->get_total_anims(), frame_count, cur_frame));
        }
      }
    }
  }
  if (!gv_selected) {
    int ret = QMessageBox::warning(
        this, tr("Geometry is not selected"),
        tr("Arrow array can be created for certain geometry. Please select a "
           "geometry in Object Inspector first!"));
  ...
  zero_descr =
      new qspoiler_widget_t(tr("All vectors will be zero"), nullptr, false);

  field_selector =
      new qspoiler_widget_t(tr("Select xgeometry fields:"), nullptr, false);
  auto field_selector_lt = new QVBoxLayout();

  std::vector<QLabel *> fs_lbl = {new QLabel(tr("vx:")), new QLabel(tr("vy:")),
                                  new QLabel(tr("vz:"))};
  ...
  connect(ok_button, &QPushButton::clicked, this,
          &create_arrow_array_widget_t::ok_button_clicked);

  connect(cancel_button, &QPushButton::clicked, this,
          &create_arrow_array_widget_t::cancel_button_clicked);

  connect(rb_zero, &QRadioButton::toggled, this,
          &create_arrow_array_widget_t::react_gb_zero_checked);

  connect(rb_field, &QRadioButton::toggled, this,
          &create_arrow_array_widget_t::react_gb_field_checked);

  connect(rb_frame, &QRadioButton::toggled, this,
          &create_arrow_array_widget_t::react_gb_frame_checked);

  connect(rb_frame_firstlast, &QRadioButton::toggled, this,
          &create_arrow_array_widget_t::react_frame_rbs_checked);

  connect(rb_frame_currnext, &QRadioButton::toggled, this,
          &create_arrow_array_widget_t::react_frame_rbs_checked);

  connect(rb_frame_numbers, &QRadioButton::toggled, this,
          &create_arrow_array_widget_t::react_frame_rbs_checked);
...
void create_arrow_array_widget_t::ok_button_clicked() {
  ...
  if (type_param_name->text() == "") {
    int ret = QMessageBox::warning(this, tr("Missing name!"),
    ...
    if (!cur_ws)
      return;
    auto nt = astate->ws_mgr->m_bhv_mgr->fbr_ws_item_by_type(
        arrow_array_view_t::get_type_static());
    ...
    if (rb_field->isChecked()) {
      int f1 = ix_fields[fs_combo[0]->currentIndex()],
...
void create_arrow_array_widget_t::cancel_button_clicked() { reject(); }
